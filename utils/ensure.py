...
def unique[T](arr: abc.Iterable[T], message: str = "") -> None:
    ...
    ensure(not dupes, message, "duplicate elements:", dupes)
...
def equal_sets[T](
    ...
    ensure(not diff, message, diff, "present in the former but not the latter")
    diff = s2.difference(s1)
    ensure(not diff, message, diff, "present in the latter but not the former")
