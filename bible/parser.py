...
def parse_unbound_biola_coptic_nt(unbound_biola_coptic_nt_tsv_path,
                                  code_to_coptic, code_to_english):
  df = pd.read_csv(unbound_biola_coptic_nt_tsv_path, sep='\t',
  ...
  for book_code, chapters in data.items():
    data[book_code] = bible.Book(code_to_english[book_code], code_to_coptic[book_code], book_code,
...
def parse_delimited_chapters_book(
    name, print_name, path, want_num_chapters, chapter_delimiter, chapter_starter,
    chapter_idx_suffix):
  def parse_chapter(text):
    ...
    if not lines[-1]:
      lines = lines[:-1]
    return bible.Chapter(idx, str(idx), 'Chapter',
                         [bible.Verse(i + 1, str(i + 1), t) for i, t in
                          enumerate(lines)])
