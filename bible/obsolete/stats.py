...
def same_finger(a, b):
  return a != b and any(a in finger and b in finger for finger in LEFT_HAND + RIGHT_HAND)
...
class pairFreq:
  ...
  def __str__(self):
    return '\t'.join(map(str, [self.p1, self.f1, self.p2, self.f2, self.s, "%.2f" % self.percentage, self.sameFinger, self.sameHand]))
