...
loc_shared () {
  echo $(( $(loc . -depth 1) + $(loc env) + $(loc xooxle) + $(loc utils) + $(loc pre-commit) + $(loc .github) + $(loc test) ))
}
...
LOC=$(loc .)
LOC_INC_ARCHIVE=$(( LOC + LOC_ARCHIVE))
...
"\n  ${YELLOW}Total including archive (broken): ${GREEN}${LOC_INC_ARCHIVE}${RESET}"
