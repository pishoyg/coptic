...
export interface Source {
   ...
   * Right now, the `innerHTML` is always a `<ul>` element, with several
   * `<li>` elements beneath it. There is no reason to store this as a string.
   * An array would do better, and the `<ul>` element could then be
   * programmatically generated from the array.
   */
...
export class Reference {
  ...
  public tooltip(): (Node | string)[] {
    ...
    if (!this.source.innerHTML) {
      return fragment;
    }
    const template: HTMLTemplateElement = document.createElement('template');
    template.innerHTML = this.source.innerHTML;
...
const DATA: Resource[] = [
  ...
  },
  {
    source: {
      ...
      innerHTML:
        '<ul><li>See <strong><a href="#BKU">BKU</a></strong>.</li></ul>',
    },
    variants: ['Kam'],
  },
  {
